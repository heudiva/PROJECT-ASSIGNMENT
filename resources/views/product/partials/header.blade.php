...
<div  class="flex flex-col md:flex-row items-stretch md:items-center md:space-x-3 space-y-3 md:space-y-0 justify-between mx-4 py-4 border-t dark:border-gray-700">
    ...
    <div class="w-full md:w-auto flex flex-col md:flex-row space-y-2 md:space-y-0 items-stretch md:items-center justify-end md:space-x-3 flex-shrink-0">
        <button  id="createProductButton" data-drawer-target="drawer-update-product" data-drawer-show="drawer-update-product" aria-controls="drawer-update-product" data-drawer-backdrop="false"  class="text-white bg-primary-700 hover:bg-primary-800 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 dark:bg-primary-600 dark:hover:bg-primary-700 focus:outline-none dark:focus:ring-primary-800" >
                Add new product
    </button>
        ...
        <div class="flex items-center space-x-3 w-full md:w-auto">
            ...
            <div id="actionsDropdown" class="hidden z-10 w-44 bg-white rounded divide-y divide-gray-100 shadow dark:bg-gray-700 dark:divide-gray-600">
                ...
                <div class="py-1">
                    <a href="#" id="delete-selected-btn" class="block py-2 px-4 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-200 dark:hover:text-white">Delete selected</a>
                </div>
            </div>
        </div>
    </div>
...
<script>
    ...
    // ដាក់ event listener លើ input search ដែលមាន id simple-search
    $("#simple-search").on("keyup", function() {
        var value = $(this).val().toLowerCase();

        // ស្វែងរកជួរទាំងអស់ក្នុងតារាង (លើកលែង header)
        $("#myTable tr").filter(function() {
            // លុបចោលជួរដេកដែលមិនត្រូវនឹងតម្លៃស្វែងរក
...
$(document).ready(function() {
    // 1. ជ្រើស/ដោះជ្រើសទាំងអស់
    $('#checkbox-all').change(function() {
        $('.checkbox').prop('checked', this.checked);
    });

    // 2. ប៊ូតុងលុបធាតុដែលបានជ្រើស
    $('#delete-selected-btn').click(function(e) {
        e.preventDefault();

        // ទាញយក ID ទាំងអស់ដែលបានជ្រើស
        const selectedIds = $('.checkbox:checked').map(function() {
            return $(this).closest('tr').data('product-id');
        }).get();

        if (selectedIds.length === 0) {
            toastr.warning('សូមជ្រើសរើសយ៉ាងហោចណាស់ ១ ធាតុ');
            return;
        }

        if (confirm(`តើអ្នកពិតជាចង់លុប ${selectedIds.length} ធាតុដែលបានជ្រើសមែនទេ?`)) {
            const $btn = $(this);
            $btn.addClass('opacity-50 cursor-not-allowed').text('កំពុងលុប...');

            // ផ្ញើ AJAX request
            $.ajax({
                type: "POST",
                url: "{{ route('admin.product.delete-multiple') }}",
                data: {
                    ids: selectedIds,
                    _token: "{{ csrf_token() }}"
                },
                success: function(response) {
                    ...
                    // លុបជួរដេកដែលបានជ្រើស
                    $('.checkbox:checked').closest('tr').fadeOut(300, function() {
                        $(this).remove();
                    });
                    $('#checkbox-all').prop('checked', false);
                },
                error: function(xhr) {
                    toastr.error(xhr.responseJSON?.message || 'កំហុសក្នុងការលុប');
                },
                complete: function() {
                    $btn.removeClass('opacity-50 cursor-not-allowed').text('Delete selected');
                }
            });
        }
    });
