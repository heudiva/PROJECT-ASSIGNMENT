...
<div id="drawer-create-product" class="fixed top-0 left-0 z-50 w-full h-screen max-w-3xl p-4 overflow-y-auto transition-transform -translate-x-full bg-white dark:bg-gray-800" tabindex="-1" aria-labelledby="drawer-create-product-label" aria-hidden="true">
    <form id="formCreateProduct">
        ...
        <button type="button" id="close-create-product-form" data-drawer-dismiss="drawer-create-product" aria-controls="drawer-create-product" class="text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 absolute top-2.5 right-2.5 inline-flex items-center dark:hover:bg-gray-600 dark:hover:text-white">
...
<script>
    $(document).ready(function() {
        $('#delete-product-create-form').on('click', function(e){
            e.preventDefault();
        ...
        $('#formCreateProduct').on('submit', function(e){
            e.preventDefault();
            var data = $('#formCreateProduct').serialize();
            alert(data);
            $.ajax({
                type: "POST",
